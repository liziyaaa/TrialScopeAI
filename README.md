...
TrialScopeAI 将方案中的自然语言入排标准转为可审核规则，在用户导入的候选队列中进行方案约束评估，比较每项条件对候选规模、数据完整性与人群构成的影响。它不是把“符合/不符合”做得更快，而是帮助医学、统计和运营人员在方案执行前看清约束与权衡。

**[在线系统](https://trialscopeai.streamlit.app/)** ｜ **[项目补充材料 PDF](output/pdf/TrialScopeAI_项目补充材料.pdf)** ｜ **[公开验证案例 GOLDEN-4](https://clinicaltrials.gov/study/NCT02347774)**

> 在线系统以空白研究工作区启动，不自动载入案例、规则或候选队列。首页使用 GOLDEN-4 展示完整业务闭环；正式工作区通过 NCT 编号、标准原文或文字型 PDF 建立研究，并由用户导入当前研究的候选队列 CSV。
...
首页公开案例采用 COPD Ⅲ期试验 [GOLDEN-4（NCT02347774）](https://clinicaltrials.gov/study/NCT02347774)。该方案同时包含年龄、吸烟史、肺功能、用药、合并症和时间窗，用于说明从方案导入、飞书审核到招募评估的完整闭环，但不会自动进入正式工作区。

## 评委 2 分钟体验路径

打开 **[在线系统](https://trialscopeai.streamlit.app/)** 后，可按以下顺序使用：

1. **首页**：了解系统解决的问题、适用角色，以及包含飞书审核的五阶段工作流；
2. **方案导入**：输入 NCT 编号、粘贴标准原文或上传文字型 PDF，核对后建立研究；
3. **标准审核**：核对结构化标准的原文、字段、阈值、单位、时间窗和执行方式；
4. **协作中心**：将标准同步至飞书，由医学与运营人员记录审核状态、意见和版本；
5. **队列评估**：下载字段模板、导入当前研究的候选队列 CSV，再运行确定性规则；
6. **情景分析**：从本研究的数值标准中选择一项，比较阈值变化前后的规模、构成与数据负担；
7. **历史记录**：保存研究并恢复上次处理阶段，无需重复导入和设置。

首页案例只用于说明系统输出，不会自动写入工作区。

## 当前已经实现

| 能力 | 当前实现 | 评委可验证的结果 |
|---|---|---|
| 方案导入 | NCT 编号、粘贴文本、文字型 PDF | 原文确认后才进入解析，不对扫描件生成猜测结果 |
| 标准结构化 | 提取字段、运算符、阈值、单位、时间窗、适用条件与原文来源 | 每项标准保留原文和来源，由人工确认后进入执行 |
| 医学审核 | 可逐条修改结构化结果，主观标准标为人工确认 | 保存后才进入候选人群仿真 |
| 飞书协同 | 标准双向审核，并保存情景快照与真实验证记录 | 只同步结构化标准和汇总数据，不上传 PDF 正文或候选者明细 |
| 确定性评估 | 导入候选队列 CSV，输出“规则符合 / 不符合 / 信息不足 / 人工复核” | 每个结论保留候选者值、标准值、原因和原文 |
| 单项标准边际影响 | 逐条暂不执行可计算标准并重跑当前队列 | 回答“移除这一约束后，候选规模与人群构成怎样变化” |
| 多目标情景权衡 | 同时比较候选规模、代表性差异和数据未决负担 | 呈现取舍，不自动建议放宽方案 |
| 结果导出 | 结构化标准 JSON/CSV、仿真结果 CSV、中英文评估摘要 Markdown | 结果可继续用于汇报与复核 |
| 研究历史 | 当前浏览器保存方案、审核状态、分析摘要和操作时间线 | 可恢复工作区；不保存 PDF 原文件或候选人明细 |
| 服务降级 | 解析服务不可用时保留已导入原文和当前工作区 | 不生成猜测规则，可稍后重试或从历史恢复 |
...
系统每次从空白工作区开始，由使用者导入当前研究方案和候选队列。候选 CSV 必须先完成合法授权与去标识化；系统不保存 PDF 原文件，也不把候选者明细同步到飞书。首页公开案例只解释产品能力，不会进入正式流程或历史记录。

## 当前验证证据

| 资产或检查 | 当前结果 | 说明 |
|---|---:|---|
| 规则金标准 | 27 条 | 覆盖类型、字段、运算符、阈值、单位、时间窗和原文来源 |
| 独立边界病例 | 50 个 | 覆盖阈值相等、缺失值、单位、时间窗、主观标准和多重失败 |
| 自动化测试 | 61 项通过 | 覆盖规则、PDF、NCT、模型响应、飞书同步、历史恢复、审核差异、边际影响、情景权衡和 Streamlit 完整路径 |
...
| 李朝元 | 成都理工大学，测控技术与仪器，本科在读（2023-2027） | 数据导入、结构化解析、规则引擎、可视化、测试和部署 |
...
- 所有情景结果均为基于当前导入队列的规则模拟，不构成临床试验方案修改建议；
...
系统默认进入空白工作区。导入方案后，可在队列评估页下载与当前规则对应的 CSV 空白模板。
...
实时解析包含会话限额、进程小时限额和文本哈希缓存；关闭后保留已导入原文，不生成猜测结果。
...
- `src/history.py`：浏览器研究工作区、操作时间线、序列化与恢复；
- `tests/`：离线自动化验收。
