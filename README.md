...
没有候选数据时，可在“队列评估”页直接下载四组虚构测试队列，分别覆盖完整混合、低风险、缺失字段和边界值路径。文件也保存在 [`sample_data/`](sample_data/)；它们不会被系统自动载入，也不包含真实患者信息。
...
“审核状态”和“执行方式”是两个独立概念：前者记录飞书协作进度（待审核、已确认、需修改、需专家复核），后者表示该条标准由确定性规则执行还是保留人工判断。飞书中的状态、审核人或修改意见发生变化时，网站会将其列为待确认差异并保存在研究历史中。
