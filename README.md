...
1. **首页**：了解系统解决的问题、适用角色和四阶段工作流；
2. **工作台**：集中查看当前研究、队列状态、约束路径、主要限制因素和人群构成；
3. **标准审核**：浏览 27 条结构化标准，核对原文、字段、阈值、单位、时间窗和执行方式；
4. **协作中心**：查看飞书协作流程、待审核数据和无配置时的审核模板；
5. **队列评估**：运行方案约束仿真，查看固定合成人群中的计算结果与逐项证据；
6. **情景分析**：比较候选规模、人群代表性和数据采集负担之间的权衡；
7. **历史记录**：保存当前研究并恢复上次处理阶段，无需重复导入和设置。
